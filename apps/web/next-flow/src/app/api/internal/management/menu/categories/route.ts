...
async function readBody(request: Request): Promise<CreateManagedCategoryInput> {
  ...
  return parsed as CreateManagedCategoryInput;
}
...
export async function POST(request: Request): Promise<Response> {
  try {
    ...
    return Response.json(
      { ok: true, data: await createManagedCategory(await readBody(request)) },
