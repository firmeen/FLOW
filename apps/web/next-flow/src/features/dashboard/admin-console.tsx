...
function OrdersView({ state }: { state: ReturnType<typeof useFoodFlow>["state"] }) {
  return <div><SectionHeading eyebrow="Traceability" title="All orders" description="Every order in the demo state with its table, value, and current internal status." />{state.orders.length === 0 ? <div className="mt-6"><EmptyState title="No orders" /></div> : <div className="mt-6 overflow-hidden rounded-lg border border-border bg-card">{[...state.orders].sort((a, b) => b.submittedAt.localeCompare(a.submittedAt)).map((order, index) => { const table = state.tables.find((candidate) => candidate.id === order.tableId); const tone = getStatusPresentation(order.status).tone; return <div className={`grid gap-3 px-4 py-4 sm:grid-cols-[90px_1fr_100px_150px] sm:items-center ${index ? "border-t border-border" : ""}`} key={order.id}><div><p className="text-sm font-bold text-foreground">{order.number}</p><p className="text-[10px] text-foreground/38">{table?.code}</p></div><p className="truncate text-xs text-foreground/55">{order.items.map((item) => `${item.quantity}x ${item.menuItemName}`).join(" / ")}</p><p className="text-sm font-bold text-foreground">{formatTHB(order.subtotal)}</p><div className="flex items-center justify-between gap-2"><StatusPill tone={tone === "accent" ? "forest" : tone}>{getStatusPresentation(order.status).label}</StatusPill><span className="text-[10px] text-foreground/35">{formatBangkokTime(order.submittedAt)}</span></div></div>; })}</div>}</div>;
}

function TablesView({ state }: { state: ReturnType<typeof useFoodFlow>["state"] }) {
  return <div><SectionHeading eyebrow="Floor overview" title="Restaurant tables" description="Twelve scannable table states linked to their current dining sessions." /><div className="mt-6 grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6">{state.tables.map((table) => { const session = table.currentSessionId ? state.tableSessions.find((candidate) => candidate.id === table.currentSessionId) : undefined; const orders = session ? session.orderIds.map((id) => state.orders.find((order) => order.id === id)).filter(Boolean) : []; const total = orders.reduce((sum, order) => sum + (order?.subtotal ?? 0), 0); const status = getStatusPresentation(table.status); return <Card className={`p-4 ${table.status === "READY" ? "border-chart-3/50 ring-1 ring-chart-3/30" : ""}`} key={table.id}><div className="flex items-start justify-between"><span className="text-xl font-bold tracking-[-0.04em] text-foreground">{table.code}</span><span className={`mt-1 size-2 rounded-full ${status.dotClassName}`} /></div><p className="mt-5 text-[10px] font-bold uppercase tracking-[0.08em] text-foreground/38">{status.label}</p>{session ? <><p className="mt-1 text-xs font-semibold text-foreground">{orders.length} order{orders.length === 1 ? "" : "s"}</p><p className="mt-3 text-sm font-bold text-foreground">{formatTHB(total)}</p></> : <p className="mt-1 text-xs text-foreground/35">Ready for guests</p>}</Card>; })}</div></div>;
}
