    ...
    <div>
      ...
      <div className="mt-6 grid gap-5 xl:grid-cols-[1.35fr_.85fr]">
        <Card className="p-5 sm:p-6">
          <div className="flex items-center justify-between"><div><p className="font-semibold text-foreground">Orders currently waiting</p><p className="mt-1 text-xs text-foreground/45">Confirmation queue across the floor</p></div><Badge tone={waiting.length ? "warning" : "success"}>{waiting.length}</Badge></div>
          <div className="mt-5">
        ...
        <Card className="p-5 sm:p-6">
          <div className="flex items-center gap-3"><span className="grid size-9 place-items-center rounded-md bg-muted text-foreground"><TrendingUp className="size-4" /></span><div><p className="font-semibold text-foreground">Top selling items</p><p className="text-xs text-foreground/42">By quantity ordered</p></div></div>
          <ol className="mt-5 space-y-3">
            {topItems.map((item, index) => <li className="flex items-center gap-3" key={item.name}><span className="grid size-7 place-items-center rounded-full bg-muted text-[10px] font-black text-muted-foreground">{index + 1}</span><div className="min-w-0 flex-1"><p className="truncate text-sm font-semibold text-foreground">{item.name}</p><p className="text-[10px] text-foreground/38">{item.quantity} sold</p></div><span className="text-xs font-bold text-foreground">{formatTHB(item.revenue)}</span></li>)}
          </ol>
        </Card>
      </div>
      ...
      <div className="mt-5 grid gap-5 xl:grid-cols-3">
        <Card className="p-5 sm:p-6">
          <div className="flex items-center gap-3"><Banknote className="size-5 text-muted-foreground" /><div><p className="font-semibold text-foreground">Payment breakdown</p><p className="text-xs text-foreground/42">Recorded revenue by method</p></div></div>
          {paymentTotal === 0 ? <div className="mt-5"><EmptyState compact title="No payments yet" /></div> : <div className="mt-5 space-y-4">{Object.entries(paymentBreakdown).sort((a, b) => b[1] - a[1]).map(([method, amount]) => <div key={method}><div className="mb-1.5 flex justify-between text-xs"><span className="font-semibold text-foreground/58">{method.replaceAll("_", " ")}</span><span className="font-bold text-foreground">{formatTHB(amount)}</span></div><div className="h-2 overflow-hidden rounded-full bg-muted"><div className="h-full rounded-full bg-foreground" style={{ width: `${Math.max(5, amount / paymentTotal * 100)}%` }} /></div></div>)}</div>}
        </Card>

        <Card className="p-5 sm:p-6">
          <div className="flex items-center justify-between"><div className="flex items-center gap-3"><XCircle className="size-5 text-destructive" /><div><p className="font-semibold text-foreground">Cancelled orders</p><p className="text-xs text-foreground/42">Rejected or cancelled today</p></div></div><Badge tone={cancelledOrders.length ? "danger" : "neutral"}>{cancelledOrders.length}</Badge></div>
          {cancelledOrders.length === 0 ? <div className="mt-5"><EmptyState compact title="No cancelled orders" /></div> : <div className="mt-5 space-y-1">{cancelledOrders.map((order, index) => { const table = state.tables.find((candidate) => candidate.id === order.tableId); return <div className={`flex items-center gap-3 py-2.5 ${index ? "border-t border-border" : ""}`} key={order.id}><span className="grid size-8 place-items-center rounded-md bg-destructive/10 text-[10px] font-black text-destructive">{table?.code}</span><div className="min-w-0 flex-1"><p className="text-xs font-semibold text-foreground">{order.number}</p><p className="truncate text-[10px] text-foreground/42">{order.rejectionReason ?? order.status.toLocaleLowerCase()}</p></div><StatusPill tone="danger">{order.status.toLocaleLowerCase()}</StatusPill></div>; })}</div>}
        </Card>
      ...
      <Card className="mt-5 p-5 sm:p-6">
        <div className="flex items-center justify-between"><div className="flex items-center gap-3"><Activity className="size-5 text-muted-foreground" /><div><p className="font-semibold text-foreground">Recent activity</p><p className="text-xs text-foreground/42">Traceable operational events</p></div></div><Badge>{state.auditEvents.length}</Badge></div>
        <div className="mt-5 grid gap-x-6 lg:grid-cols-2">
          {activity.map((event, index) => <div className={`flex gap-3 py-2.5 ${index > 1 ? "border-t border-border" : ""}`} key={event.id}><span className="mt-1.5 size-2 shrink-0 rounded-full bg-muted-foreground" /><div className="min-w-0 flex-1"><p className="text-xs font-semibold text-foreground">{event.action.replaceAll("_", " ").toLocaleLowerCase()}</p><p className="mt-0.5 truncate text-[10px] text-foreground/40">{event.entityType} - {event.reason ?? event.entityId}</p></div><time className="shrink-0 text-[10px] text-foreground/35">{formatBangkokTime(event.timestamp)}</time></div>)}
        </div>
      </Card>
    </div>
  );
