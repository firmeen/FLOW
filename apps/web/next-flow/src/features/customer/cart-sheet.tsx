...
export function CartSheet({
  ...
  return (
    <div className="fixed inset-0 z-50 flex justify-end bg-[#0b2119]/55 backdrop-blur-[2px]" role="presentation" onMouseDown={(event) => !submitting && event.target === event.currentTarget && onClose()}>
      <div ref={panelRef} tabIndex={-1} className="animate-slide-up flex h-full w-full max-w-md flex-col bg-[#fbfaf6] shadow-2xl outline-none" role="dialog" aria-modal="true" aria-labelledby="cart-title">
        <header className="flex items-center justify-between border-b border-line px-5 py-4">
          <div>
            <p className="text-[10px] font-bold uppercase tracking-[0.14em] text-sage">{tableLabel}</p>
            <h2 id="cart-title" className="mt-0.5 text-xl font-semibold tracking-[-0.03em] text-forest">Your order</h2>
          </div>
          ...
          {lines.length === 0 ? (
            ...
            <div className="space-y-3">
              {lines.map((line) => {
                const item = menuItems.find((candidate) => candidate.id === line.menuItemId);
                if (!item) return (
                  <article className="rounded-lg border border-[#e8b8b3] bg-[#fff5f3] p-4" key={line.id}>
                    <p className="text-sm font-bold text-[#8e3d36]">Unavailable menu item</p>
                    <p className="mt-1 text-xs text-[#8e3d36]/70">This saved item is no longer on the menu. Remove it before sending.</p>
                ...
                return (
                  <article className="rounded-lg border border-line bg-white p-3.5" key={line.id}>
                    <div className="flex gap-3">
                      <FoodImage className="size-20 shrink-0 rounded-md" imageKey={item.imageKey ?? item.imageUrl} alt={item.name} />
                      <div className="min-w-0 flex-1">
                        <div className="flex items-start justify-between gap-3">
                          <h3 className="text-sm font-semibold leading-5 text-forest">{item.name}</h3>
                          <p className="shrink-0 text-sm font-bold text-forest">{formatTHB(getCartLineTotal(line, menuItems, modifierGroups))}</p>
                        </div>
                        {choices.length > 0 && <p className="mt-1 text-[11px] leading-4 text-foreground/45">{choices.join(" / ")}</p>}
                        {line.specialRequest && <p className="mt-1 text-[11px] italic leading-4 text-status-amber">&ldquo;{line.specialRequest}&rdquo;</p>}
                      </div>
                    </div>
                    <div className="mt-3 flex items-center justify-between border-t border-line pt-3">
                      ...
                      <div className="flex items-center gap-1">
                        <button className="px-2 py-1 text-xs font-semibold text-forest underline-offset-2 hover:underline disabled:opacity-40" type="button" disabled={submitting} onClick={() => onEdit(line.id)}>Edit</button>
        ...
        <footer className="border-t border-line bg-white px-5 pb-[max(1.25rem,env(safe-area-inset-bottom))] pt-4">
          <div className="mb-4 flex items-center justify-between">
            ...
            <p className="text-xl font-bold tracking-[-0.03em] text-forest">{formatTHB(subtotal)}</p>
          </div>
          <Button
            fullWidth
            size="lg"
          ...
          {error && <p className="mt-2 rounded-md bg-[#fff0ee] px-3 py-2 text-center text-xs font-semibold text-[#8e3d36]" role="alert">{error}</p>}
