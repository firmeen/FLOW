...
const TERMINAL_ORDER_STATUSES = new Set(["REJECTED", "CANCELLED", "CLOSED"]);
...
function commandMessage(code: string): string {
  switch (code) {
    ...
    case "CUSTOMER_COMMAND_CART_NOT_EDITABLE":
      return "That cart has already been sent. A fresh table cart is ready for the next order.";
    case "CUSTOMER_COMMAND_CONFLICT":
      return "The table state changed while you were using it. Refresh and try again.";
    default:
...
function menuImageStyle(imageUrl: string | null): CSSProperties | undefined {
  return imageUrl ? { backgroundImage: `url(${JSON.stringify(imageUrl)})` } : undefined;
}

async function requestActiveCart(): Promise<DurableCartAggregate> {
  const response = await fetch("/api/customer/cart", {
    method: "POST",
    credentials: "same-origin",
    headers: { Accept: "application/json" },
  });
  return readApi<DurableCartAggregate>(response);
}

async function requestOrders(): Promise<readonly CustomerVisibleOrder[]> {
  ...
  return readApi<readonly CustomerVisibleOrder[]>(response);
}
    ...
    <div className="mt-4 grid grid-cols-5 gap-1" aria-label="Order progress">
      {stages.map((stage) => (
        <div key={stage.key} className="min-w-0">
          ...
          <p
            className={`mt-1.5 truncate text-[9px] font-bold uppercase tracking-[0.08em] ${stage.active ? "text-foreground" : "text-muted-foreground/60"}`}
          >
            {stage.label}
          </p>
        </div>
...
function MenuArtwork({ item }: { readonly item: DurableCustomerMenuItem }) {
  return (
    <div className="relative h-44 overflow-hidden bg-gradient-to-br from-stone-100 via-zinc-100 to-stone-200 dark:from-zinc-900 dark:via-stone-900 dark:to-zinc-950">
      {item.imageUrl ? (
        <div
          className="h-full w-full bg-cover bg-center transition duration-500 group-hover:scale-[1.025]"
          style={menuImageStyle(item.imageUrl)}
          role="img"
          aria-label={item.name}
        />
      ) : (
        <div className="grid h-full place-items-center">
          <Sparkles className="size-6 text-muted-foreground/35" aria-hidden="true" />
        </div>
      )}
      ...
      <div className="absolute bottom-3 left-3 flex gap-1.5">
        {item.badges.slice(0, 2).map((badge) => (
          <span
            key={badge.id}
            className="rounded-full border border-white/15 bg-zinc-950/65 px-2.5 py-1 text-[9px] font-bold uppercase tracking-[0.09em] text-white backdrop-blur"
          >
            {badge.name}
          </span>
        ))}
...
export function DurableCustomerExperience({
  snapshot,
}: {
  readonly snapshot: DurableCustomerSnapshot;
}) {
  ...
  const visibleItems = useMemo(() => {
    ...
    return [...menu.items]
      ...
      .filter(
        (item) =>
          !term ||
          `${item.name} ${item.thaiName ?? ""} ${item.description}`
            .toLocaleLowerCase()
            .includes(term),
      )
  ...
  const activeOrder =
    orders.find((order) => !TERMINAL_ORDER_STATUSES.has(order.status)) ?? orders[0] ?? null;
  ...
  const ensureCart = useCallback(async (): Promise<DurableCartAggregate> => {
    ...
    const promise = requestActiveCart();
  ...
  useEffect(() => {
    let active = true;
    void Promise.all([requestActiveCart(), requestOrders()])
      ...
      .catch((error: unknown) => {
        if (!active) return;
        setPageError(
          error instanceof Error ? error.message : "The table experience could not be loaded.",
        );
      })
      .finally(() => {
  ...
  async function submitOrder() {
    ...
    try {
      ...
      const [nextCart, nextOrders] = await Promise.all([requestActiveCart(), requestOrders()]);
      setCart(nextCart);
      setOrders(nextOrders);
      setCartOpen(false);
      setView("orders");
      ...
      >
        <div className="text-center">
          ...
          <p className="mt-1 text-xs text-muted-foreground">
            Connecting the live menu and order session.
          </p>
        </div>
      </main>
    );
  ...
  return (
    ...
    >
      {notice && (
        <div
          className="fixed left-1/2 top-4 z-[100] flex max-w-[calc(100vw-2rem)] -translate-x-1/2 items-center gap-2 rounded-full border border-white/10 bg-zinc-950 px-4 py-2.5 text-xs font-semibold text-white shadow-2xl"
          role="status"
        >
        ...
        <div className="mx-auto flex h-16 max-w-6xl items-center justify-between gap-4 px-4 sm:px-6">
          <div className="flex min-w-0 items-center gap-3">
            {view !== "home" && (
              <button
                type="button"
                className="grid size-9 shrink-0 place-items-center rounded-xl border border-border bg-card transition hover:bg-muted focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
            ...
            >
              <span className="block truncate text-sm font-semibold tracking-[-0.025em]">
                {storefront.restaurantName}
              </span>
              <span className="mt-0.5 flex items-center gap-1.5 text-[9px] font-bold uppercase tracking-[0.13em] text-muted-foreground">
                <span
                  className={`size-1.5 rounded-full ${storefront.isOpen ? "bg-emerald-500" : "bg-zinc-400"}`}
                />
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              aria-label="Call staff unavailable until secure service requests are connected"
              className="grid size-9 place-items-center rounded-xl border border-border bg-card text-muted-foreground opacity-55 sm:flex sm:w-auto sm:gap-2 sm:px-3 sm:text-[11px] sm:font-semibold"
            >
            ...
            <button
              type="button"
              className="relative grid size-9 place-items-center rounded-xl bg-foreground text-background shadow-sm transition hover:opacity-90 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2"
              onClick={() => setCartOpen(true)}
              aria-label={`Open cart with ${cartCount} items`}
            >
              <ShoppingBag className="size-4" />
              {cartCount > 0 && (
                <span className="absolute -right-1.5 -top-1.5 grid min-w-5 place-items-center rounded-full border-2 border-background bg-foreground px-1 text-[9px] font-bold leading-4 text-background">
                  {cartCount}
                </span>
              )}
            </button>
        ...
        <section className="mx-auto max-w-6xl px-4 py-8 sm:px-6 sm:py-12">
          <div className="grid gap-4 lg:grid-cols-[1.45fr_0.55fr]">
            <div className="relative overflow-hidden rounded-[2rem] bg-zinc-950 px-6 py-8 text-white shadow-[0_32px_100px_rgb(0_0_0/0.18)] sm:px-9 sm:py-10">
              ...
              <div className="absolute -bottom-28 -left-20 size-64 rounded-full bg-white/[0.035] blur-3xl" />
              <div className="relative">
                <div className="flex flex-wrap items-center gap-2">
                  <StatusPill className="border-white/12 bg-white/8 text-white" dot>
                    {storefront.isOpen ? "Kitchen open" : "Currently closed"}
                  </StatusPill>
                  <span className="rounded-full border border-white/10 px-2.5 py-1 text-[10px] font-bold uppercase tracking-[0.14em] text-white/55">
                    {storefront.tableLabel}
                  </span>
                </div>
                <p className="mt-10 text-[10px] font-bold uppercase tracking-[0.2em] text-white/45">
                  Your table. Your pace.
                </p>
                <h1 className="mt-3 max-w-xl text-4xl font-semibold tracking-[-0.055em] sm:text-5xl">
                  Order beautifully.
                  <br />
                  Everything stays connected.
                </h1>
                <p className="mt-5 max-w-lg text-sm leading-6 text-white/60">
                  Browse the live menu, save your cart directly to this table, and follow each
                  order from confirmation to service.
                </p>
            ...
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-1">
              <button
                type="button"
                onClick={() => setView("orders")}
                className="group rounded-[1.75rem] border border-border bg-card p-5 text-left shadow-[0_18px_55px_rgb(0_0_0/0.05)] transition hover:-translate-y-0.5 hover:border-foreground/20 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
              >
                ...
                <p className="mt-2 text-xs leading-5 text-muted-foreground">
                  {activeOrder
                    ? `${activeOrder.orderNumber} · ${formatTHB(minorToMajor(activeOrder.subtotalMinor))}`
              ...
              <button
                type="button"
                onClick={() => setCartOpen(true)}
                className="group rounded-[1.75rem] border border-border bg-muted/45 p-5 text-left transition hover:-translate-y-0.5 hover:border-foreground/20 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
              >
                <div className="flex items-center justify-between">
                  <span className="grid size-10 place-items-center rounded-2xl border border-border bg-background">
                    <ShoppingBag className="size-4" />
                  </span>
                ...
                <p className="mt-1 text-lg font-semibold tracking-[-0.03em]">
                  {cartCount ? "Ready when you are" : "Start with the menu"}
                </p>
                <p className="mt-2 text-xs leading-5 text-muted-foreground">
                  Cart changes are persisted to this verified table session.
                </p>
              </button>
            </div>
          </div>
      ...
      {view === "menu" && (
        <section
          ...
          data-flow-live-menu="database"
        >
          <div className="flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between">
            <div>
              <p className="text-[10px] font-bold uppercase tracking-[0.18em] text-muted-foreground">
                Live from {storefront.branchName}
              </p>
            ...
            <label className="relative block w-full sm:max-w-xs">
              <span className="sr-only">Search the live menu</span>
          ...
          <div className="-mx-4 mt-7 overflow-x-auto px-4 pb-2 sm:-mx-6 sm:px-6">
            <div className="flex min-w-max gap-2" aria-label="Menu categories">
              <button
                type="button"
          ...
          {visibleItems.length === 0 ? (
            ...
            <div className="mt-7 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              {visibleItems.map((item) => (
                <article
                  key={item.id}
                  data-flow-menu-item={item.id}
                  className="group overflow-hidden rounded-[1.6rem] border border-border/80 bg-card shadow-[0_16px_50px_rgb(0_0_0/0.045)] transition hover:-translate-y-0.5 hover:border-foreground/15 hover:shadow-[0_24px_65px_rgb(0_0_0/0.075)] focus-within:ring-2 focus-within:ring-ring"
                >
                  <button
                    type="button"
                    ...
                    <div className="p-4 sm:p-5">
                      <div className="flex items-start justify-between gap-4">
                        ...
                        <p className="shrink-0 text-sm font-semibold tabular-nums">
                          {formatTHB(minorToMajor(item.basePriceMinor))}
                        </p>
                      </div>
                      ...
                      <div className="mt-4 flex items-center justify-between border-t border-border/70 pt-3">
                        <span className="text-[10px] font-bold uppercase tracking-[0.11em] text-muted-foreground">
                          {item.modifierGroups.length ? "Options available" : "Ready as listed"}
                        </span>
      ...
      {view === "orders" && (
        <section
          ...
          data-flow-customer-orders="database"
        >
          <div className="flex items-end justify-between gap-4">
            ...
            <Button
              ...
              size="sm"
              disabled={busy}
              leftIcon={<RefreshCcw className={`size-3.5 ${busy ? "animate-spin" : ""}`} />}
              onClick={() => void refreshLiveState()}
            >
              Refresh
            </Button>
          ...
          {orders.length === 0 ? (
            <div className="mt-12">
              <EmptyState
                ...
                title="No orders yet"
                description="When you send your first order, its live restaurant status will stay here."
                action={
                  <Button size="sm" onClick={() => setView("menu")}>
                    Open menu
                  </Button>
                }
              />
            ...
            <div className="mt-7 space-y-3">
              {orders.map((order, index) => (
                <article
                  key={order.id}
                  data-flow-customer-order={order.id}
                  className={`rounded-[1.6rem] border p-5 sm:p-6 ${
                    index === 0
                      ? "border-foreground/16 bg-card shadow-[0_20px_65px_rgb(0_0_0/0.06)]"
                ...
                >
                  <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                    <div>
                      ...
                      <p className="mt-1 text-xs text-muted-foreground">
                        {order.submittedAt
                          ? `Sent ${formatBangkokTime(order.submittedAt)}`
                          : "Submission time unavailable"}
                      </p>
                    </div>
                    <p className="text-xl font-semibold tracking-[-0.035em] tabular-nums">
                      {formatTHB(minorToMajor(order.subtotalMinor))}
                    </p>
                  </div>
      ...
      <nav
        className="fixed inset-x-0 bottom-0 z-30 border-t border-border/70 bg-background/90 pb-[max(0.6rem,env(safe-area-inset-bottom))] pt-2 backdrop-blur-xl sm:hidden"
