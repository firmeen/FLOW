...
export function ProductSheet({ item, modifierGroups, badges, initialLine, onClose, onAdd }: ProductSheetProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-[#0b2119]/55 p-0 backdrop-blur-[2px] sm:items-center sm:p-5" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && onClose()}>
      <div ref={panelRef} tabIndex={-1} className="animate-slide-up max-h-[94vh] w-full overflow-y-auto rounded-t-2xl bg-[#fbfaf6] shadow-2xl outline-none sm:max-w-2xl sm:rounded-xl" role="dialog" aria-modal="true" aria-labelledby="product-title">
        <div className="relative h-52 sm:h-64">
          ...
          <IconButton className="absolute right-4 top-4 bg-white/95 shadow" label="Close product details" onClick={onClose}>
            <X className="size-4" />
          </IconButton>
        </div>
        ...
        <div className="px-5 pb-28 pt-5 sm:px-7 sm:pb-7">
          <div className="flex items-start justify-between gap-5">
            <div>
              ...
              <h2 id="product-title" className="text-2xl font-semibold tracking-[-0.035em] text-forest">{item.name}</h2>
              {item.thaiName && <p className="mt-1 text-sm text-foreground/55">{item.thaiName}</p>}
            </div>
            <p className="text-xl font-bold text-forest">{formatTHB(item.basePrice)}</p>
          </div>
            ...
            {groups.map((group) => (
              <fieldset key={group.id}>
                <div className="mb-3 flex items-center justify-between gap-4">
                  <legend className="font-semibold text-forest">{group.name}{group.required && <span className="ml-1 text-status-red">*</span>}</legend>
                ...
                <div className="overflow-hidden rounded-lg border border-line bg-white">
                  {group.choices.filter((choice) => choice.active).map((choice, index) => {
                    ...
                    return (
                      <label className={`flex cursor-pointer items-center gap-3 px-4 py-3.5 ${index > 0 ? "border-t border-line" : ""}`} key={choice.id}>
                        <input
                          className="sr-only"
                        ...
                        />
                        <span className={`grid size-5 place-items-center ${group.maximumSelections === 1 ? "rounded-full" : "rounded"} border ${checked ? "border-forest bg-forest text-white" : "border-line bg-white"}`}>
          ...
          <label className="mt-7 block">
            <span className="text-sm font-semibold text-forest">Special request</span>
            <span className="ml-2 text-[11px] text-foreground/40">Optional</span>
            <textarea
              className="mt-2 min-h-20 w-full resize-none rounded-lg border border-line bg-white px-3.5 py-3 text-sm outline-none transition focus:border-forest"
          ...
          <div className="fixed inset-x-0 bottom-0 border-t border-line bg-white/95 p-4 backdrop-blur sm:static sm:mt-6 sm:border-0 sm:bg-transparent sm:p-0">
