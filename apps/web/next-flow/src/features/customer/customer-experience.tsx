...
export function CustomerExperience({ tableCode }: { tableCode: string }) {
  ...
  return (
    <main className="min-h-screen bg-[#f6f5ef] pb-24 text-foreground">
      {serviceMessage && (
        <div className="fixed left-1/2 top-4 z-[70] flex -translate-x-1/2 items-center gap-2 rounded-full bg-forest px-4 py-2.5 text-xs font-semibold text-white shadow-xl" role="status">
          <CheckCircle2 className="size-4 text-lime" /> {serviceMessage}
        </div>
      )}

      <header className="sticky top-0 z-30 border-b border-line bg-[#fbfaf6]/95 backdrop-blur">
      ...
      {view === "landing" && (
        <section className="mx-auto max-w-xl px-5 py-10 sm:py-16">
          <div className="overflow-hidden rounded-xl bg-forest text-white ff-shadow">
            ...
            <div className="p-6 sm:p-8">
              <div className="flex items-center justify-between gap-4">
                <StatusPill className="border-white/15 bg-white/10 text-white" dot>{branch?.isOpen ? "Open now" : "Closed"}</StatusPill>
              ...
              <Button className="mt-7 bg-lime text-forest hover:bg-[#d2e86b]" size="lg" fullWidth onClick={() => setView("menu")} rightIcon={<ChevronRight className="size-4" />}>View menu</Button>
            </div>
          </div>
          <div className="mt-4 grid grid-cols-2 gap-3">
            <button className="ff-panel rounded-lg p-4 text-left transition hover:border-forest/35" onClick={() => sendService("CALL_STAFF")}>
              <Bell className="size-5 text-status-amber" />
              <p className="mt-3 text-sm font-semibold text-forest">Call staff</p>
              <p className="mt-1 text-[11px] leading-4 text-foreground/45">Send a request to the floor team.</p>
            </button>
            <button className="ff-panel rounded-lg p-4 text-left transition hover:border-forest/35" onClick={() => setView("session")}>
              <ReceiptText className="size-5 text-status-blue" />
              <p className="mt-3 text-sm font-semibold text-forest">Current order</p>
          ...
          <section className="border-b border-line bg-white">
            <div className="mx-auto max-w-6xl px-4 py-7 sm:px-6 sm:py-9">
              <div className="flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between">
                <div>
                  <p className="text-[10px] font-bold uppercase tracking-[0.16em] text-sage">Fresh from our kitchen</p>
                  <h1 className="mt-1 text-3xl font-semibold tracking-[-0.045em] text-forest">What would you like?</h1>
                </div>
                <label className="relative block w-full sm:max-w-xs">
                  <Search className="absolute left-3 top-1/2 size-4 -translate-y-1/2 text-foreground/35" />
                  <span className="sr-only">Search menu</span>
                  <input className="h-11 w-full rounded-lg border border-line bg-[#f8f7f2] pl-10 pr-3 text-sm outline-none transition focus:border-forest" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search dishes or drinks" />
                </label>
              </div>
            </div>
          </section>
          <nav className="sticky top-16 z-20 border-b border-line bg-[#fbfaf6]/95 backdrop-blur" aria-label="Menu categories">
            <div className="no-scrollbar mx-auto flex max-w-6xl gap-1 overflow-x-auto px-4 py-2.5 sm:px-6">
              {[{ id: "all", name: "All" }, ...categories].map((category) => (
                <button key={category.id} className={`shrink-0 rounded-full px-4 py-2 text-xs font-semibold transition ${categoryId === category.id ? "bg-forest text-white" : "text-foreground/55 hover:bg-forest-soft hover:text-forest"}`} onClick={() => setCategoryId(category.id)}>{category.name}</button>
              ))}
            </div>
          </nav>
      ...
      {view === "session" && (
        <section className="mx-auto max-w-2xl px-4 py-7 sm:px-6 sm:py-10">
          {lastOrderId && (
            <div className="mb-5 flex items-start gap-3 rounded-lg border border-[#b9dbc7] bg-[#edf8f1] p-4 text-[#245b3d]">
          ...
          <div className="flex items-end justify-between gap-5">
            <div>
              <p className="text-[10px] font-bold uppercase tracking-[0.16em] text-sage">Live table session</p>
              <h1 className="mt-1 text-3xl font-semibold tracking-[-0.04em] text-forest">Table {table.code.replace("T", "")}</h1>
          ...
          <div className="mt-6 space-y-4">
            {sessionOrders.length === 0 ? (
              ...
              <article className={`ff-panel rounded-lg p-5 ${order.id === lastOrderId ? "ring-2 ring-[#a9cbb7]" : ""}`} key={order.id}>
                <div className="flex items-start justify-between gap-4">
                  <div><p className="text-sm font-bold text-forest">Order {order.number}</p><p className="mt-1 text-[11px] text-foreground/42">Sent {formatBangkokTime(order.submittedAt)}</p></div>
                ...
                {order.rejectionReason && <div className="mb-4 rounded-md border border-[#e8b8b3] bg-[#fff7f5] px-3 py-2.5 text-xs leading-5 text-[#7f3933]"><strong className="mr-1">Restaurant note:</strong>{order.rejectionReason}</div>}
                <div className="border-t border-line pt-4">
                  {order.items.map((item) => (
                    <div className="flex justify-between gap-4 py-1 text-sm" key={item.id}><span className="text-foreground/68"><strong className="mr-2 text-forest">{item.quantity}x</strong>{item.menuItemName}</span><span className="font-semibold text-forest">{formatTHB(item.lineTotal)}</span></div>
                  ))}
                </div>
              </article>
            ))}
      ...
      {view === "menu" && cartCount > 0 && (
        <button className="fixed bottom-24 left-1/2 z-30 flex w-[calc(100%-2rem)] max-w-md -translate-x-1/2 items-center justify-between rounded-xl bg-forest px-4 py-3.5 text-white shadow-[0_16px_36px_rgba(9,38,27,0.3)]" onClick={() => setCartOpen(true)}>
          <span className="flex items-center gap-3"><span className="grid size-7 place-items-center rounded-full bg-lime text-xs font-black text-forest">{cartCount}</span><span className="text-sm font-semibold">View cart</span></span>
          <span className="text-sm font-bold">{formatTHB(cartSubtotal)}</span>
        </button>
      )}

      <nav className="fixed inset-x-0 bottom-0 z-20 border-t border-line bg-white/96 pb-[env(safe-area-inset-bottom)] backdrop-blur" aria-label="Customer navigation">
...
function MenuCard({ item, badges, available, onSelect }: { item: MenuItem; badges: { id: string; name: string }[]; available: boolean; onSelect: () => void }) {
  return (
    <article className={`group overflow-hidden rounded-lg border border-line bg-white ${available ? "transition hover:-translate-y-0.5 hover:border-forest/25 hover:shadow-[0_12px_28px_rgba(20,52,39,0.08)]" : "opacity-72"}`}>
      <button className="block w-full text-left" disabled={!available} onClick={onSelect}>
        <div className="relative h-44 overflow-hidden">
          ...
          {!available && <span className="absolute inset-0 grid place-items-center bg-[#17211b]/25"><span className="rounded-full bg-white px-3 py-1.5 text-xs font-bold uppercase tracking-[0.08em] text-forest">{item.status === "SOLD_OUT" ? "Sold out" : "Available on selected days"}</span></span>}
        </div>
        <div className="p-4">
          <div className="flex items-start justify-between gap-4">
            <div className="min-w-0"><h2 className="text-base font-semibold tracking-[-0.02em] text-forest">{item.name}</h2>{item.thaiName && <p className="mt-0.5 truncate text-xs text-foreground/43">{item.thaiName}</p>}</div>
            <span className="shrink-0 text-base font-bold text-forest">{formatTHB(item.basePrice)}</span>
          </div>
          <p className="mt-2 line-clamp-2 text-xs leading-5 text-foreground/52">{item.description}</p>
          <div className="mt-4 flex items-center justify-between"><span className="flex items-center gap-1.5 text-[10px] font-medium text-foreground/42"><Clock3 className="size-3" /> {item.estimatedPreparationMinutes} min</span><span className={`grid size-8 place-items-center rounded-full ${available ? "bg-forest text-white" : "bg-surface-muted text-foreground/35"}`}><Plus className="size-4" /></span></div>
        </div>
      </button>
    </article>
  );
...
function CustomerNav({ active, icon, label, onClick }: { active: boolean; icon: React.ReactElement<{ className?: string }>; label: string; onClick: () => void }) {
  return <button className={`flex flex-col items-center justify-center gap-1 text-[10px] font-semibold ${active ? "text-forest" : "text-foreground/38"}`} onClick={onClick}>{<span className={active ? "text-forest" : "text-foreground/38"}>{icon}</span>}<span>{label}</span></button>;
}
